...
class CovidTool:
    ...
    def loadit(self):
        if self.df.empty: return
        plt.figure(); self.df.set_index("date")[["cases","recovered"]].plot(kind="area", alpha=0.5); plt.title("AREA view"); plt.tight_layout(); plt.savefig("covid_02_area.png"); plt.close()
        plt.figure(); self.df.groupby("country")[["cases","deaths"]].sum().plot(kind="bar"); plt.title("BAR view 2"); plt.tight_layout(); plt.savefig("covid_02_bar.png"); plt.close()

def build_map(app):
    return {"1": lambda: app.wash(input("Path (blank -> sample): ") or "covid.csv"),
            "2": app.look,
            "3": lambda: (app.drawit(), print("tidied")),
            "4": app.quickstats,
            "5": lambda: (app.loadit(), print("saved images"))}
